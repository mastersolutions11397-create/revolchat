...
export default function PlansPage() {
  ...
  const plans: Plan[] = [
    ...
    },
    {
      ...
      features: [
        { name: `${PLAN_CONFIGS.starter.credits.toLocaleString()} Yetti Tokens`, included: true },
    ...
    },
    {
      ...
      features: [
        { name: `${PLAN_CONFIGS.growth.credits.toLocaleString()} Yetti Tokens`, included: true },
    ...
    },
    {
      ...
      features: [
        { name: `${PLAN_CONFIGS.pro.credits.toLocaleString()} Yetti Tokens`, included: true },
    ...
    },
    {
      ...
      features: [
        { name: `${PLAN_CONFIGS.enterprise.credits.toLocaleString()} Yetti Tokens`, included: true },
  ...
  const handlePlanSelect = async (planKey: string) => {
    ...
    console.log('PlansPage: User email:', user?.email, 'Effective Workspace ID:', effectiveWorkspaceId);
    ...
    try {
      ...
      console.log('PlansPage: Checkout session API response status:', response.status);
      const { sessionId, error } = await response.json();
      console.log('PlansPage: Checkout session response data:', { sessionId, error });
  ...
  console.log('PlansPage: Loading state:', loading, 'Current plan:', currentPlan);
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto animate-fade-in-up">
      ...
      <div className="space-y-6">
        ...
        {plans.filter(plan => !isCurrentPlan(plan.planKey)).map((plan, index) => {
          const Icon = plan.icon;
          const isCurrent = isCurrentPlan(plan.planKey);
          console.log(`PlansPage: Rendering plan ${index + 1}/${plans.length}:`, plan.name, 'isCurrent:', isCurrent);
          ...
          return (
            ...
            >
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start text-sm">
                    ...
                    <span className={feature.included ? 'text-slate-900' : 'text-slate-400 line-through'}>
                      {feature.name}
                    </span>
                  </li>
                ))}
              ...
              {isCurrent ? (
                ...
                <button
                  onClick={() => {
                    console.log('PlansPage: Plan button clicked for plan:', plan.name, 'planKey:', plan.planKey);
                    if (plan.planKey) {
